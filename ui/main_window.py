...
class MainWindow(QMainWindow):
    ...
    def _on_edit_collection(self, collection_id):
        ...
        dialog = EditCollectionDialog(collection["name"], collection["icon"], self)

        if dialog.exec():
            data = dialog.get_data()
            self.data_manager.update_collection(collection_id, data["name"], data["icon"])
    ...
    def _on_edit_page(self, page_id):
        ...
        if dialog.exec():
            self.data_manager.rename_page(self.active_collection_id, page_id, dialog.get_name())
    ...
    def _on_delete_page(self, page_id):
        ...
        if dialog.exec() == dialog.DialogCode.Accepted:
            self.data_manager.delete_page(self.active_collection_id, page_id)
            self._show_collection(self.data_manager.get_collection(self.active_collection_id))
